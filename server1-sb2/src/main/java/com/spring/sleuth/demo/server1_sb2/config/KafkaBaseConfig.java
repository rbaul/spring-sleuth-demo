...
import com.spring.sleuth.demo.server1_sb2.config.custom.CustomRecordInterceptor;
...
public abstract class KafkaBaseConfig {
    ...
    protected <T> ConcurrentKafkaListenerContainerFactory<String, T> createKafkaListenerContainer(Class<T> targetType, String groupName, int concurrency) {
        ...
        factory.setRecordInterceptor(new CustomRecordInterceptor<>()); // Additional way to add interceptor on receive
        return factory;
    }
}